...
def load_pretrain_weight(pretrain_dir, model_no_ddp):
    """
    Resume if checkpoint is available.
    Return
    - epoch of loaded checkpoint.
    ...
    last_checkpoint = os.path.join(pretrain_dir, "checkpoint.pth")
    if not os.path.isfile(last_checkpoint):
        return epoch, best_val_metrics
    ...
    print(
        f"Loaded model state at {epoch}. Loaded best val metrics so far."
    )
