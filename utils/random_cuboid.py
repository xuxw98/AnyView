...
class RandomCuboid_AnyView(object):
    """
    RandomCuboid augmentation from DepthContrast [https://arxiv.org/abs/2101.02691]
    We slightly modify this operation to account for object detection.
    This augmentation randomly crops a cuboid from the input and
    ...
    def __call__(self, pcs, target_boxes, per_point_labels=None):
        point_cloud = pcs.reshape(-1, pcs.shape[-1])
        point_cloud_50000 = pc_util.random_sampling(
            point_cloud, 50000, return_choices=False
        )

        range_xyz = np.max(point_cloud_50000[:, 0:3], axis=0) - np.min(
            point_cloud_50000[:, 0:3], axis=0
        )
        ...
        for _ in range(100):
            ...
            sample_center = point_cloud_50000[np.random.choice(len(point_cloud_50000)), 0:3]
            ...
            upper_idx = (
                np.sum((point_cloud_50000[:, 0:3] <= max_xyz).astype(np.int32), 1) == 3
            )
            lower_idx = (
                np.sum((point_cloud_50000[:, 0:3] >= min_xyz).astype(np.int32), 1) == 3
            )
            new_pointidx = (upper_idx) & (lower_idx)
            ...
            new_point_cloud_50000 = point_cloud_50000[new_pointidx, :]
            ...
            new_point_cloud[~new_pointidx, :] = 0
            ...
            if self.box_filter_policy == "center":
                ...
                if (
                    ...
                    box_centers = target_boxes[:, 0:3]
                    new_pc_min_max = np.min(new_point_cloud_50000[:, 0:3], axis=0), np.max(
                        new_point_cloud_50000[:, 0:3], axis=0
                    )
                    keep_boxes = np.logical_and(
                        np.all(box_centers >= new_pc_min_max[0], axis=1),
                        np.all(box_centers <= new_pc_min_max[1], axis=1),
                ...
                if per_point_labels is not None:
                    ...
                    new_per_point_labels = None
                # if we are here, all conditions are met. return boxes
                return new_point_cloud.reshape(pcs.shape), new_boxes, new_per_point_labels

        # fallback
        return pcs, target_boxes, per_point_labels