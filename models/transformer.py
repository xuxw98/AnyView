...
class TransformerEncoder(nn.Module):
    ...
    def compute_mask(self, xyz):
        mask = torch.zeros(xyz.shape[0], xyz.shape[1], xyz.shape[1]).to(xyz.get_device())
        zero_index = (xyz.sum(-1) == 0)
        mask = zero_index.unsqueeze(-2).repeat(1,xyz.shape[1],1)
        self_index = (torch.eye(xyz.shape[1]).to(xyz.get_device()) == 0)
        mask2 = self_index.unsqueeze(0).repeat(xyz.shape[0],1,1)
        mask = mask & mask2
        return mask
...
class MaskedTransformerEncoder(TransformerEncoder):
    ...
    def compute_mask(self, xyz, radius, dist=None):
        with torch.no_grad():
            ...
            mask = dist >= radius
        mask2 = torch.zeros(xyz.shape[0], xyz.shape[1], xyz.shape[1]).to(xyz.get_device())
        zero_index = (xyz.sum(-1) == 0)
        mask2 = zero_index.unsqueeze(-2).repeat(1,xyz.shape[1],1)
        mask = mask | mask2
        self_index = (torch.eye(xyz.shape[1]).to(xyz.get_device()) == 0)
        mask3 = self_index.unsqueeze(0).repeat(xyz.shape[0],1,1)
        mask = mask & mask3
        return mask, dist
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if idx == 0 and self.interim_downsampling:
                ...
                if pos is not None:
                    ...
                    pos = gather_operation(pos, xyz_inds)
