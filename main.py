...
from utils.io import save_checkpoint, resume_if_possible, load_pretrain_weight
...
def make_args_parser():
    ...
    parser.add_argument("--image_lr", default=5e-4, type=float)
    ...
    parser.add_argument(
        ...
        choices=["3detr", "3detr_sepview", "3detr_sepview2"],
    )
    ### Encoder
    parser.add_argument(
    ...
    parser.add_argument("--no_pcenc", default=False, action="store_true")
    parser.add_argument("--DQ_FPS", default=False, action="store_true")
    parser.add_argument("--enc_PE", default=False, action="store_true")
    parser.add_argument("--enc_proj", default=False, action="store_true")
    ...
    ##### Dataset #####
    parser.add_argument(
        "--dataset_name", required=True, type=str, choices=["scannet", "scannetSV", "scannetAV", "sunrgbd"]
    )
    parser.add_argument(
        "--dataset_root_dir",
        type=str,
    ...
    parser.add_argument("--use_depth_fusion", default=False, action="store_true")
    parser.add_argument("--use_view_drop", default=False, action="store_true")
    parser.add_argument("--use_random_cuboid", default=False, action="store_true")
    parser.add_argument("--num_views", default=30, type=int)
    parser.add_argument("--continue_view", default=False, action="store_true")
    parser.add_argument("--without_trans", default=False, action="store_true")
    ...
    parser.add_argument("--test_anyview", default=-1, type=int)

    ##### I/O #####
    parser.add_argument("--pretrain_dir", default=None, type=str)
    parser.add_argument("--head_only", default=False, action="store_true")
...
def do_train(
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        if epoch % args.eval_every_epoch == 0 or epoch == (args.max_epoch - 1):
            ...
            if is_primary() and (
                len(best_val_metrics) == 0 or (best_val_metrics[0.25]["mAP"] + best_val_metrics[0.5]["mAP"] < ap25 + ap50)
            ):
...
def main(local_rank, args):
    ...
    if args.test_only:
        ...
        if loaded_epoch < 0 and args.pretrain_dir is not None:
            load_pretrain_weight(args.pretrain_dir, model_no_ddp)
            if args.head_only:
                # If pretrain, only finetune the detection head. (For ScanNet<-->SUNRGBD)
